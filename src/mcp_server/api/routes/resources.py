...
async def _load_file_from_disk(file_path: str) -> str:
    ...
    return await FileLoader.load_file(
        file_path=file_path,
        validate_base_dir=False,  # Already validated in calling function
    )
...
def _validate_resource_path(
    ...
    """
    Validate resource file path (existence and path traversal check).

    Args:
        file_path: Path to resource file
        resource_uri: MCP resource URI (for error messages)
    ...
    Raises:
        ...
        ForbiddenError: If path traversal detected
    """
    # Check file exists
...
async def load_resource_file(
    ...
    """
    Load resource file from disk asynchronously (without caching).

    Used exclusively for testing. Production code should use load_resource_file_cached.
    ...
    # Validate file path (existence and path traversal)
    _validate_resource_path(file_path, resource_uri, base_dir)

    # Read file directly (no caching)
    try:
        content = await _load_file_from_disk(str(file_path))
...
async def _load_resource_with_metrics(
    file_path: Path,
    resource_uri: str,
    resource_type: str,
    cache_key: str,
    ...
    """
    Load resource with end-to-end latency metrics.

    Helper function that wraps load_resource_file_cached with performance monitoring.
...
async def get_pattern_resource(
    ...
    # Load resource with caching and metrics
    cache_key = f"resource:patterns:{validated_language}:{validated_name}"
    return await _load_resource_with_metrics(
        file_path=file_path,
        resource_uri=resource_uri,
