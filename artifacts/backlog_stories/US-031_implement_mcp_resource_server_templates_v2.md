...
After implementation, generators can request `mcp://resources/templates/prd` and receive prd-template.xml content from the server, eliminating per-project duplication.
...
3. **Extend Resource Handler (FastAPI):**
   ```python
